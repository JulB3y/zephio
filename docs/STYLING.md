...
Use `TUI_COLOR_INDEX(n)` for the standard 256-color terminal palette (values 0-255):

```c
TuiColor fg = TUI_COLOR_INDEX(15);  // Bright white
TuiColor bg = TUI_COLOR_INDEX(4);   // Blue
```
...
// ... through TUI_COLOR_BRIGHT_WHITE (15)
...
Use `TUI_COLOR_RGB(r, g, b)` for precise color control:

```c
TuiColor fg = TUI_COLOR_RGB(255, 100, 50);   // Orange
TuiColor bg = TUI_COLOR_RGB(30, 30, 46);     // Catppuccin background
```
...
Use `TUI_COLOR_NONE` for transparent/no-color:

```c
TuiColor none = TUI_COLOR_NONE;
```
...
|---|---|---|
| `TUI_ATTR_NONE` | 0x0000 | No attributes |
| `TUI_ATTR_BOLD` | 0x0001 | Bold/bright |
| `TUI_ATTR_DIM` | 0x0002 | Dim/faint |
| `TUI_ATTR_ITALIC` | 0x0004 | Italic |
| `TUI_ATTR_UNDERLINE` | 0x0008 | Underline |
| `TUI_ATTR_BLINK` | 0x0010 | Blinking |
| `TUI_ATTR_REVERSE` | 0x0020 | Reverse video (swap fg/bg) |
| `TUI_ATTR_STRIKETHROUGH` | 0x0040 | Strikethrough |
...
```c
// Using the macro
TuiStyle style = TUI_STYLE(15, 4, TUI_ATTR_BOLD);
// fg=white, bg=blue, bold

// RGB style
TuiStyle rgb_style = TUI_STYLE_RGB(255, 100, 50, 30, 30, 46, TUI_ATTR_NONE);

// Using the function
TuiStyle s = tui_style_make(TUI_COLOR_INDEX(15), TUI_COLOR_INDEX(0), TUI_ATTR_BOLD);
...
|---|---|---|
| `TUI_STATE_NORMAL` | 0 | Default state |
| `TUI_STATE_FOCUSED` | 1 | Widget has keyboard focus |
| `TUI_STATE_DISABLED` | 2 | Widget is disabled (`widget->disabled = 1`) |
| `TUI_STATE_ACTIVE` | 3 | Widget is being pressed/activated |
| `TUI_STATE_HOVER` | 4 | Mouse cursor is over the widget |
...
```c
TuiStyle normal   = TUI_STYLE(15, 234, TUI_ATTR_NONE);
TuiStyle focused  = TUI_STYLE(0,  12,  TUI_ATTR_BOLD);
TuiStyle disabled = TUI_STYLE(8,  232, TUI_ATTR_DIM);
TuiStyle active   = TUI_STYLE(0,  10,  TUI_ATTR_BOLD);
...
1. If `widget->disabled` → `TUI_STATE_DISABLED`
2. If `widget->focused` → `TUI_STATE_FOCUSED`
3. If `widget->hovered` → `TUI_STATE_HOVER`
4. Otherwise → `TUI_STATE_NORMAL`
...
tui_label_set_attr(&label, TUI_ATTR_BOLD);
...
- `TUI_COLOR_INDEX(n)` always uses 256-color sequences (`ESC[38;5;Nm`).
- `TUI_COLOR_RGB(r,g,b)` uses truecolor sequences (`ESC[38;2;R;G;Bm`) when available, or falls back to the closest 256-color index.
- You can mix indexed and RGB colors freely — each cell stores its own color type.

### Best Practices

1. Use `TUI_COLOR_INDEX()` for broad compatibility.
2. Use `TUI_COLOR_RGB()` for precise branding or gradient effects.
...
```c
TuiColor bar_fg = TUI_COLOR_INDEX(15);
TuiColor bar_bg = TUI_COLOR_INDEX(4);

tui_screen_fill(row, 0, cols, 1, " ", bar_fg, bar_bg, TUI_ATTR_BOLD);
tui_screen_write(row, 1, "NORMAL", bar_fg, bar_bg, TUI_ATTR_BOLD);

tui_screen_write(row, 20, "line 42, col 17",
    TUI_COLOR_INDEX(15), TUI_COLOR_INDEX(4), TUI_ATTR_NONE);

char pos[32];
snprintf(pos, sizeof(pos), "%d:%d", cols, rows);
tui_screen_write(row, cols - strlen(pos) - 1, pos,
    bar_fg, bar_bg, TUI_ATTR_BOLD);
```

### Colored Box Panel

```c
TuiStyle panel = TUI_STYLE(0, 236, TUI_ATTR_NONE);
tui_style_fill(row, col, width, height, " ", &panel);
tui_screen_box_single(row, col, width, height,
    TUI_COLOR_INDEX(12), TUI_COLOR_INDEX(236), TUI_ATTR_NONE);
tui_style_write(row + 1, col + 1, "Panel Title",
    &(TuiStyle){ .fg = TUI_COLOR_INDEX(14), .bg = TUI_COLOR_INDEX(236),
                 .attr = TUI_ATTR_BOLD });
```
...
for (int i = 0; i < width; i++) {
    ...
    tui_screen_set_cell(row, col + i, " ",
        TUI_COLOR_RGB(r, g, b), TUI_COLOR_RGB(r, g, b), TUI_ATTR_NONE);
}
```
