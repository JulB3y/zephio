/**
 * @file zephio_text_view.h
 * @brief Multi-line text display widget with word-wrapping and scrolling.
 *
 * ZephioTextView displays read-only text with automatic word-wrapping and
 * scroll container integration. Supports keyboard (arrows, PageUp/Down,
 * Home/End) and mouse-wheel scrolling via the embedded ZephioScrollContainer.
 */

#ifndef ZEPHIO_TEXT_VIEW_H
#define ZEPHIO_TEXT_VIEW_H

#include "zephio_scroll_container.h"

/**
 * @brief Multi-line text display widget.
 */
typedef struct {
    ZephioScrollContainer base;
    char  *text;
    int    text_len;
    int   *line_starts;
    int   *line_lengths;
    int    line_count;
    int    line_capacity;
    int    max_line_width;
    ZephioColor fg;
    ZephioColor bg;
    ZephioAttr  attr;
    int    word_wrap;
} ZephioTextView;

/**
 * @brief Initialize a text view widget with context.
 *
 * @param tv      TextView struct to initialize.
 * @param ctx     TUI context.
 * @param x       Column offset.
 * @param y       Row offset.
 * @param width   Width in columns.
 * @param height  Height in rows.
 * @return ZEPHIO_OK on success.
 */
ZephioResult zephio_text_view_init_ctx(ZephioTextView *tv, ZephioContext *ctx, int x, int y,
                               int width, int height);

/**
 * @brief Set the display text (copied).
 *
 * Recomputes word-wrapped lines and updates scroll bounds.
 *
 * @param tv    TextView widget.
 * @param text  New text to display (may be NULL).
 */
void zephio_text_view_set_text(ZephioTextView *tv, const char *text);

/**
 * @brief Set foreground and background colors.
 */
void zephio_text_view_set_colors(ZephioTextView *tv, ZephioColor fg, ZephioColor bg);

/**
 * @brief Set text attributes (bold, underline, etc.).
 */
void zephio_text_view_set_attr(ZephioTextView *tv, ZephioAttr attr);

/**
 * @brief Enable or disable word-wrapping (default: enabled).
 *
 * When disabled, lines are split only at newline characters and
 * horizontal scrolling becomes available for long lines.
 */
void zephio_text_view_set_word_wrap(ZephioTextView *tv, int enabled);

/**
 * @brief Return the current number of wrapped lines.
 */
int zephio_text_view_get_line_count(ZephioTextView *tv);

/**
 * @brief Return the current scroll Y offset.
 */
int zephio_text_view_get_scroll_y(ZephioTextView *tv);

#endif
