...
void tui_toast_render(TuiContext *ctx, TuiToastManager *mgr, int screen_rows, int screen_cols)
{
    ...
    for (int i = 0; i < TUI_TOAST_MAX_COUNT; i++) {
        ...
        tui_screen_fill(ctx, t->row, t->col, t->width, TUI_TOAST_HEIGHT,
        ...
        tui_screen_set_cell(ctx, t->row + TUI_TOAST_HEIGHT - 1, t->col,
                            "\xe2\x95\x9a", use_border, use_bg, base_attr);
        for (int c = 1; c < t->width - 1; c++) {
            tui_screen_set_cell(ctx, t->row + TUI_TOAST_HEIGHT - 1,
                                t->col + c, "\xe2\x95\x90",
                                use_border, use_bg, base_attr);
        }
        tui_screen_set_cell(ctx, t->row + TUI_TOAST_HEIGHT - 1,
