                            ...
                            const char **icon)
{
    ...
    case TUI_TOAST_SUCCESS:
        *fg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_BLACK);
        *bg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_GREEN);
        *border_fg = ZEPHIO_COLOR_INDEX(TUI_COLOR_BRIGHT_GREEN);
        *icon      = "\xe2\x9c\x93";
        break;
    case TUI_TOAST_WARNING:
        *fg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_BLACK);
        *bg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_YELLOW);
        *border_fg = ZEPHIO_COLOR_INDEX(TUI_COLOR_BRIGHT_YELLOW);
        *icon      = "\xe2\x9a\xa0";
        break;
    case TUI_TOAST_ERROR:
        *fg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_BRIGHT_WHITE);
        *bg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_RED);
        *border_fg = ZEPHIO_COLOR_INDEX(TUI_COLOR_BRIGHT_RED);
        *icon      = "\xe2\x9c\x97";
        break;
    default:
        *fg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_BRIGHT_WHITE);
        *bg        = ZEPHIO_COLOR_INDEX(TUI_COLOR_BLUE);
        *border_fg = ZEPHIO_COLOR_INDEX(TUI_COLOR_BRIGHT_CYAN);
        *icon      = "\xe2\x84\xb9";
        break;
    }
}
...
void tui_toast_render(TuiContext *ctx, TuiToastManager *mgr, int screen_rows, int screen_cols)
{
    ...
    for (int i = 0; i < ZEPHIO_TOAST_MAX_COUNT; i++) {
        ...
        int dim = 0;
        TuiAttr base_attr = ZEPHIO_ATTR_NONE;
        if (opacity < 0.33) {
            dim = 1;
        ...
        if (dim) {
            use_fg     = ZEPHIO_COLOR_INDEX(TUI_COLOR_GRAY_MID);
            use_bg     = ZEPHIO_COLOR_INDEX(TUI_COLOR_BG_DARK);
            use_border = ZEPHIO_COLOR_INDEX(TUI_COLOR_GRAY_DARK);
        }

        tui_screen_fill(ctx, t->row, t->col, t->width, ZEPHIO_TOAST_HEIGHT,
                        " ", use_fg, use_bg, base_attr);

        tui_screen_set_cell(ctx, t->row, t->col, "\xe2\x95\x94",
                            use_border, use_bg, base_attr);
        for (int c = 1; c < t->width - 1; c++) {
            tui_screen_set_cell(ctx, t->row, t->col + c, "\xe2\x95\x90",
                                use_border, use_bg, base_attr);
        }
        tui_screen_set_cell(ctx, t->row, t->col + t->width - 1, "\xe2\x95\x97",
                            use_border, use_bg, base_attr);

        for (int r = 1; r < ZEPHIO_TOAST_HEIGHT - 1; r++) {
            tui_screen_set_cell(ctx, t->row + r, t->col, "\xe2\x95\x91",
                                use_border, use_bg, base_attr);
            tui_screen_set_cell(ctx, t->row + r, t->col + t->width - 1,
                                "\xe2\x95\x91", use_border, use_bg, base_attr);
        }

        tui_screen_set_cell(ctx, t->row + ZEPHIO_TOAST_HEIGHT - 1, t->col,
                            "\xe2\x95\x9a", use_border, use_bg, base_attr);
        for (int c = 1; c < t->width - 1; c++) {
            tui_screen_set_cell(ctx, t->row + ZEPHIO_TOAST_HEIGHT - 1,
                                t->col + c, "\xe2\x95\x90",
                                use_border, use_bg, base_attr);
        }
        tui_screen_set_cell(ctx, t->row + ZEPHIO_TOAST_HEIGHT - 1,
        ...
        tui_screen_write(ctx, text_row, t->col + 1, icon,
        ...
        if (max_msg > 0) {
            ...
            buf[msg_len] = '\0';
            tui_screen_write(ctx, text_row, msg_col, buf,
                             use_fg, use_bg, base_attr);
        }
