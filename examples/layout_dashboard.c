                     ...
                     TuiColor fg, TuiColor bg)
{
    ...
    tui_screen_fill(ctx, w->abs_y, w->abs_x, w->width, w->height,
                    " ", fg, bg, TUI_ATTR_BOLD);
    if (w->width > 2) {
        tui_screen_write(ctx, w->abs_y, w->abs_x + 1, text, fg, bg, TUI_ATTR_BOLD);
    }
}
...
static void draw_frame(TuiContext *ctx, int rows, int cols)
{
    ...
    draw_bar(ctx, &toolbar, "  File  Edit  View  Tools  Help", TUI_COLOR_INDEX(0), TUI_COLOR_INDEX(252));

    draw_panel(ctx, &nav, "Navigation", TUI_COLOR_INDEX(7), TUI_COLOR_INDEX(194));
    draw_panel(ctx, &editor, "Editor [Fill w=2]", TUI_COLOR_INDEX(0), TUI_COLOR_INDEX(234));
    draw_panel(ctx, &preview, "Preview [Fill w=1]", TUI_COLOR_INDEX(0), TUI_COLOR_INDEX(230));
