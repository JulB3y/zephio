...
typedef struct {
    ...
    ZephioLabel   title;
    ZephioLabel   section_slide;
    ZephioLabel   section_loading;
    ZephioLabel   hint;
    ZephioLabel   status;

    ZephioLabel   spinner_label;
    ZephioLabel   pulse_label;
    ZephioLabel   pulse_bar;

    ZephioContainer slide_panel;
    ZephioLabel     slide_panel_title;
    ZephioLabel     slide_panel_hint;

    ZephioContainer fade_panel;

    ZephioContainer color_block;
    ZephioLabel     color_block_text;

    ZephioSpinner spinner;
    ZephioPulse   pulse;
...
static void update_status(AppWidgets *w, const char *msg)
{
    ...
    zephio_label_set_colors(&w->status,
                         ZEPHIO_COLOR_INDEX(15), ZEPHIO_COLOR_INDEX(236));
}

static void build_widgets(AppWidgets *w, ZephioContext *ctx, int rows, int cols)
{
    zephio_widget_init_ctx(&w->root, 0, 0, cols, rows, NULL, ctx, NULL);

    zephio_label_init_ctx(&w->title, ctx, 2, 1, cols - 4, 1,
    ...
    zephio_label_init_ctx(&w->section_slide, ctx, 2, 3, 30, 1,
                       "Slide Effects (keys 1-4):");
    zephio_label_set_colors(&w->section_slide,
                         ZEPHIO_COLOR_INDEX(12), ZEPHIO_COLOR_INDEX(0));
    zephio_label_set_attr(&w->section_slide, ZEPHIO_ATTR_BOLD);
    zephio_widget_add_child(&w->root, &w->section_slide.base);

    zephio_label_init_ctx(&w->section_loading, ctx, 2, 5, 30, 1,
                        "Loading Animations (s/p/c):");
    zephio_label_set_colors(&w->section_loading,
                         ZEPHIO_COLOR_INDEX(12), ZEPHIO_COLOR_INDEX(0));
    zephio_label_set_attr(&w->section_loading, ZEPHIO_ATTR_BOLD);
    zephio_widget_add_child(&w->root, &w->section_loading.base);

    zephio_label_init_ctx(&w->spinner_label, ctx, 4, 6, 20, 1, "Spinner: stopped");
    zephio_label_set_colors(&w->spinner_label,
                         ZEPHIO_COLOR_INDEX(7), ZEPHIO_COLOR_INDEX(0));
    zephio_widget_add_child(&w->root, &w->spinner_label.base);

    zephio_label_init_ctx(&w->pulse_label, ctx, 4, 7, 20, 1, "Pulse:   stopped");
    zephio_label_set_colors(&w->pulse_label,
                         ZEPHIO_COLOR_INDEX(7), ZEPHIO_COLOR_INDEX(0));
    zephio_widget_add_child(&w->root, &w->pulse_label.base);

    zephio_label_init_ctx(&w->pulse_bar, ctx, 4, 8, 20, 1, "");
    zephio_label_set_colors(&w->pulse_bar,
                         ZEPHIO_COLOR_INDEX(0), ZEPHIO_COLOR_INDEX(234));
    zephio_widget_add_child(&w->root, &w->pulse_bar.base);

    zephio_label_init_ctx(&w->hint, ctx, 2, 10, cols - 4, 1,
    ...
    zephio_container_init_ctx(&w->slide_panel, ctx, panel_x, panel_y, panel_w, panel_h);
    zephio_container_set_bg(&w->slide_panel, ZEPHIO_COLOR_INDEX(236));
    w->slide_panel.base.visible = 0;
    w->panel_home_x = panel_x;
    w->panel_home_y = panel_y;
    zephio_widget_add_child(&w->root, &w->slide_panel.base);

    zephio_label_init_ctx(&w->slide_panel_title, ctx, 2, 1, panel_w - 4, 1,
                       "Sliding Panel — auto-dismiss");
    zephio_label_set_colors(&w->slide_panel_title,
                         ZEPHIO_COLOR_INDEX(15), ZEPHIO_COLOR_INDEX(236));
    zephio_label_set_attr(&w->slide_panel_title, ZEPHIO_ATTR_BOLD);
    zephio_widget_add_child(&w->slide_panel.base, &w->slide_panel_title.base);

    zephio_label_init_ctx(&w->slide_panel_hint, ctx, 2, 3, panel_w - 4, 1,
                       "Wait for auto-dismiss...");
    zephio_label_set_colors(&w->slide_panel_hint,
                         ZEPHIO_COLOR_INDEX(8), ZEPHIO_COLOR_INDEX(236));
    zephio_widget_add_child(&w->slide_panel.base, &w->slide_panel_hint.base);
    ...
    zephio_container_init_ctx(&w->fade_panel, ctx, fade_x, fade_y, fade_w, fade_h);
    zephio_container_set_bg(&w->fade_panel, ZEPHIO_COLOR_INDEX(236));
    w->fade_panel.base.visible = 0;
    zephio_widget_add_child(&w->root, &w->fade_panel.base);

    {
        ...
        zephio_container_init_ctx(&w->color_block, ctx, cb_x, cb_y, cb_w, cb_h);
        zephio_container_set_bg(&w->color_block, ZEPHIO_COLOR_RGB(28, 28, 28));
        w->color_block.base.visible = 0;
        zephio_widget_add_child(&w->root, &w->color_block.base);

        zephio_label_init_ctx(&w->color_block_text, ctx, 2, 1, cb_w - 4, 1,
                           "Press 'c' to animate this block");
        zephio_label_set_colors(&w->color_block_text,
                             ZEPHIO_COLOR_RGB(210, 210, 210), ZEPHIO_COLOR_RGB(28, 28, 28));
        zephio_widget_add_child(&w->color_block.base, &w->color_block_text.base);
    }
    ...
    zephio_spinner_init(&w->spinner, &w->spinner_label.base,
                     "Spinner: ", ZEPHIO_COLOR_INDEX(14), ZEPHIO_COLOR_INDEX(0));

    zephio_pulse_init(&w->pulse, &w->pulse_bar.base, 20,
...
static void trigger_slide(AppWidgets *w, ZephioSlideDirection dir, const char *name)
{
    ZephioAnimator *animator = zephio_app_get_animator(g_app);
    if (!animator) return;

    if (w->slide_anim_id != ZEPHIO_ANIMATOR_INVALID_ID) {
        zephio_animator_remove(animator, w->slide_anim_id);
    ...
    zephio_widget_set_position(&w->slide_panel.base,
    ...
    w->slide_anim_id = zephio_effect_slide_in(animator,
                                           &w->slide_panel.base,
                                           dir, 400);
}
...
static void trigger_fade(AppWidgets *w)
{
    ...
    if (w->fade_anim_id != ZEPHIO_ANIMATOR_INVALID_ID) {
        zephio_animator_remove(animator, w->fade_anim_id);
    ...
    if (!w->fade_panel_visible) {
        ...
        w->fade_anim_id = zephio_effect_fade_in(animator,
                                             &w->fade_panel.base, 400);
    } else {
        update_status(w, "Fading overlay out...");
        w->fade_anim_id = zephio_effect_fade_out(animator,
...
static void toggle_spinner(AppWidgets *w)
{
    ...
    if (w->spinner_active) {
        zephio_spinner_stop(animator, &w->spinner);
        w->spinner_active = 0;
        zephio_label_set_text(&w->spinner_label, "Spinner: stopped");
        zephio_label_set_colors(&w->spinner_label,
                             ZEPHIO_COLOR_INDEX(7), ZEPHIO_COLOR_INDEX(0));
        update_status(w, "Spinner stopped");
    } else {
        zephio_spinner_start(animator, &w->spinner, 120);
        w->spinner_active = 1;
        zephio_label_set_colors(&w->spinner_label,
...
static void toggle_pulse(AppWidgets *w)
{
    ...
    if (w->pulse_active) {
        zephio_pulse_stop(animator, &w->pulse);
        w->pulse_active = 0;
        zephio_label_set_text(&w->pulse_label, "Pulse:   stopped");
        zephio_label_set_colors(&w->pulse_label,
                             ZEPHIO_COLOR_INDEX(7), ZEPHIO_COLOR_INDEX(0));
        update_status(w, "Pulse stopped");
    } else {
        zephio_pulse_start(animator, &w->pulse, 1.0);
        w->pulse_active = 1;
        zephio_label_set_text(&w->pulse_label, "Pulse:   ");
        zephio_label_set_colors(&w->pulse_label,
                             ZEPHIO_COLOR_INDEX(12), ZEPHIO_COLOR_INDEX(0));
...
static void color_block_fade_update(double progress, void *user_data)
{
    ...
    ZephioColor bg = zephio_color_lerp(fd->from_bg, fd->to_bg, progress);
    ZephioColor fg = zephio_color_lerp(fd->from_fg, fd->to_fg, progress);
    zephio_container_set_bg(fd->block, bg);
    zephio_label_set_colors(fd->label, fg, bg);
}
...
static void trigger_color_fade(AppWidgets *w)
{
    ...
    if (w->color_fade_anim_id != ZEPHIO_ANIMATOR_INVALID_ID) {
        zephio_animator_remove(animator, w->color_fade_anim_id);
        w->color_fade_anim_id = ZEPHIO_ANIMATOR_INVALID_ID;
    }
    ...
    if (w->color_fade_state == 0) {
        ...
        w->color_fade_anim_id = zephio_animator_create(
            animator, 800, ZEPHIO_EASE_IN_OUT_SINE,
            color_block_fade_update, color_block_fade_complete, fd);
        if (w->color_fade_anim_id != ZEPHIO_ANIMATOR_INVALID_ID)
            zephio_animator_play(animator, w->color_fade_anim_id);
