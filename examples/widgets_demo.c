...
{
    snprintf(g_w.status_text, sizeof(g_w.status_text),
             " Tab: next focus | Shift+Tab: prev | q/Esc: quit | List: Up/Down+Enter");
    tui_label_set_text(&g_w.status, g_w.status_text);
}
...
static void on_hello_click(TuiWidget *widget, void *user_data)
{
    ...
    if (name && name[0]) {
        snprintf(g_w.action_text, sizeof(g_w.action_text),
                 "Hello, %s!", name);
    } else {
        snprintf(g_w.action_text, sizeof(g_w.action_text),
                 "Hello, World!");
...
static void on_name_change(TuiWidget *widget, const char *text, void *user_data)
{
    (void)widget;
    (void)user_data;
    snprintf(g_w.action_text, sizeof(g_w.action_text),
...
static void on_name_submit(TuiWidget *widget, const char *text, void *user_data)
{
    (void)widget;
    (void)user_data;
    snprintf(g_w.action_text, sizeof(g_w.action_text),
                           ...
                           const char *item, void *user_data)
{
    ...
    (void)user_data;
    (void)index;
    snprintf(g_w.selection_text, sizeof(g_w.selection_text),
             "Selected: %s", item);
...
static void build_widgets(int rows, int cols)
{
    ...
    if (y < rows - 7) {
        int half = uw / 2 - 1;
        tui_button_init(&g_w.btn_hello, ux, y, half > 6 ? half : 6, 1,
                        "Say Hello");
        ...
        int b2x = ux + half + 2;
        if (b2x + 6 < ux + uw) {
            tui_button_init(&g_w.btn_clear, b2x, y, uw - half - 2, 1,
                            "Clear");
