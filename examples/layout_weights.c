...
static void draw_panel(TuiWidget *w, const char *label, PanelColor c)
{
    ...
    if (w->height >= 2 && w->width > 2) {
        tui_screen_write(w->abs_y + 1, w->abs_x + 1, dim,
...
static void draw_frame(int rows, int cols)
{
    ...
    for (int i = 0; i < 5; i++) {
        draw_panel(&panels[i], labels[i], colors[i]);
    }
